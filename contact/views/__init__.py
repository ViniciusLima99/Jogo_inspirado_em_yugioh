...
import random
from ..model import Player, default_cardlist
from ..forms import PlayerForm
from django.urls import reverse
from ..villains import villain1
from ..forms import BotaoForm, UserForm
...
def index(request):
    if request.method == "POST":
        ...
        if User.objects.filter(username=username).exists():
            ...
            if user is not None:
                login(request, user)
                if not Player.objects.filter(username=user.username).exists():
                    player = Player(username=user.username)
                    player.save()
                return redirect('contact:history')
            else:
                user_form.add_error('password', 'Senha incorreta ou usuário já existente.')
        else:
...
def showdeck(request):
    if request.user.is_authenticated:
        ...
        player = get_object_or_404(Player, username=request.user.username)    
...
def batalha1(request):
    ...
    if 'recomecar' in request.GET:
        player.cardlist = default_cardlist()
